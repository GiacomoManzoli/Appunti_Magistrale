...
$$fv(<l =M>) = fv(M) \qquad \text{e} \qquad fv( M \text{ match } \{\text{case }l_i = x_i => M_i \:^{i = 1\ldots n}\}) = fv(M) \cup \bigcup_{i = 1 \ldots n}\bigg( fv(M_i) \setminus \{x_i\} \bigg) $$

\noindent Servono inoltre dei nuovi valori finali e dei tipi:

\begin{align*}
	v ::=& <l = v> \vbar \ldots \\
	T ::=& < l_i : T_i \:^{i = 1 \ldots n}>
\end{align*}
...
\begin{itemize}
	...
	\begin{prooftree}
		\AxiomC{$M \rightarrow M'$}
		\UnaryInfC{$M \text{ match } \{\text{case } l_i = x_i => M_i \:^{i = 1 \ldots n}\}$}
	\end{prooftree}
