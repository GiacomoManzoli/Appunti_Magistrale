...
\begin{itemize}
	\item \textsc{(True)} $\Gamma , x : S \vdash \true : \Bool$, in questo caso applicando la sostituzione il termine \text{true} non cambia e quindi la tesi è trivialmente soddisfatta. Lo stesso vale per gli assiomi relativi a \text{false} e \text{nat}.
...
\begin{itemize}
	...
	\item \textsc{(App)}: \todo{guardare dalle note, da notare che @25 minuti c'è la crafa che spiega lo schema generico.} 
\end{itemize}
