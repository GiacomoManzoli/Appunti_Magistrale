...
\begin{itemize}
	...
	\begin{enumerate}
		...
		$$
		\item \textit{Scelta della variabile entrante per il cambio di base}: scelgo una qualsiasi di quelle fuori base a costo ridotto $\overbrace{c}_{F_{j}}$ negativo, in modo da migliorare la FO. Se ho più scelte conviene prendere la variabile con indice minore per evitare di ciclare.
	...
	Risolto questo problema con il metodo del simplesso, se $w^* = 0$, il problema di partenza non è ammissibile e non ha senso risolverlo.Se $w^* > 0$. Il tableau finale individua una possibile soluzione ammissibile per il problema di partenza, è necessario portare fuori base le $y$ e poi creare un nuovo tableau in forma canonica per il problema di partenza utilizzando come base quella appena ottenuta.
